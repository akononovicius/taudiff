...
def tau_series_generator(
    *,
    ...
    Implementation of a Markovian point process:
        tau[k+1] = tau[k] + sigma^2 * (1/(2*H) - 1) / tau[k] +
                          + sigma * epsi[k] .
    In the above tau_i is the coordinate of the point process
    (inter-event time), H is the "faked" Hurst index and epsi[k] is a
    sample of uncorrelated Gaussian noise. This process generates tau_i
    distribution, which is the same as one generated by the fractional
    point process (in the limit of small tau[k]):
    ...
    Input:
        ...
        hurst: (default: 0.5)
            ...
            with fGn). It is advisable to use hurst>=0.5.
        tau_0: (default: 0.5)
    ...
    Examples:
        ```
        >> from taudiff import powerlaw_model
        >> tsg = powerlaw_model(sigma=1e-2, hurst=0.75, n_points=16,
            seed=123)
        >> print(next(tsg))
            0.48907
        >> print(next(tsg))
    ...
    for _ in range(0, n_points):
        tau = tau + drift_const / tau + diff_const * np.random.normal()
